...
def get_out2():
    ...
    shutil.move("train_" + output_title, output_folder)
    shutil.move("test_" + output_title, output_folder)
    shutil.move("eval_" + output_title, output_folder)
    exit(0)
...
size = 500
root.geometry(str(size + 200) + "x" + str(size + 400) + "+5+5")
...
quiter = Label(root, text='TO QUIT BEFORE COMPLETION:\nCTRL + C (when clicked into command window)', foreground='white',
               background='black')
...
def output_csv(engine_list, output_filename, prf_titles, prf_col):
    ...
    out2 = open("train_" + output_filename, 'w')
    out3 = open("test_" + output_filename, 'w')
    out4 = open("eval_" + output_filename, 'w')
    ...
    out2.write("Error Classifier \n")
    out3.write("Error Classifier \n")
    out4.write("Error Classifier \n")
    ...
    for enginer in engine_list:
        ...
        for flight in enginer.flight_list:
            ...
            if flight.final_e.severity == '2':
                ...
                if flight.designation <= train_size:
                    out2.write('Grounding\n')
                if train_size < flight.designation <= (train_size + test_size):
                    out3.write('Grounding\n')
                if flight.designation > (train_size + test_size):
                    out4.write('Grounding\n')

            else:
                out1.write("Non-grounding\n")

                if flight.designation <= train_size:
                    out2.write("Non-grounding\n")
                if train_size < flight.designation <= (train_size + test_size):
                    out3.write("Non-grounding\n")
                if flight.designation > (train_size + test_size):
                    out4.write("Non-grounding\n")
    ...
    # update label to indicate completion
    printer.configure(text='Printing to: "' + "./" + output_filename.split(".")[0] + " results/" + '" Folder Complete. ({:,}'.format(
...
if __name__ == "__main__":
    print("The Labeler v4\n\n")
    moder = 1
    
    # # get valid input indicating which mode that will be used
    # keep_going = True
    # while keep_going:
    #     moder = input("Input Labeler Mode to run: ")
    #     if moder == '1' or moder == '2' or moder == '3' or moder == '4':
    #         print("Selected mode: " + moder)
    #         keep_going = False
    #     else:
    ...
    #     a = input("Please Enter the Flight Instance File (PRF): ")
    #     b = input("Please Enter the Error Instance File (FLT): ")
    #     c = input("Please Enter the Error Code File: ")
